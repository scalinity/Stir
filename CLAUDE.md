...
- `notification_schedule_rollback_failed` (SCA-309, expanded SCA-318/SCA-369): emitted from `NotificationSchedulerKit.addWithRollback` on TWO terminal-failure branches (SCA-369 split): (a) primary add throws AND rollback re-add throws (regression — user HAD a schedule and we lost it), (b) primary add throws AND no prior to restore (fresh-user first-schedule failure). Properties `{scheduler_id, identifier, error_description, prior_existed}`. `scheduler_id` ∈ `{leftovers_followup, use_soon, reactivation}` — `reactivation` was added by SCA-318 when ReactivationScheduler migrated off the legacy unobservable rollback path; SCA-360 typed it as a Swift enum. `identifier` is the failing `UNNotificationRequest.identifier`; `error_description` is the OS-supplied `NSError.localizedDescription` from the rollback throw. `prior_existed` (SCA-369) — `true` on regression branch (user lost a schedule), `false` on cold-start branch (first-time schedule failed; nothing was lost) — distinguishes the two for PostHog dashboards. Counts/strings/bools only — no user content per ADR 0009. Single-success branches do NOT emit. Adding a new scheduler that delegates to the kit is a wire-contract change — update `NotificationSchedulerKit.swift` callers, spec §15 clarification, and this list in the same commit.
