...
- **Step-8 `ops_admin_rpcs_test.ts` — one test red in isolation.** `stir_ops_list_voice_sessions: aggregates by trace_id + sorts by token use` fails when running `ops_admin_rpcs_test.ts` alone (21/22 in 430ms). Not cross-test pollution — isolates identically. Outside the step-6 voice-path audit scope (ops-admin is step-8 agent's workstream). Filed for step-8 agent to diagnose and fix in their next sweep. Owner-step: step-8 agent.
