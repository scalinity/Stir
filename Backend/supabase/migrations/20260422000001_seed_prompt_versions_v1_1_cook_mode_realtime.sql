-- 20260422000001 — cook_mode_realtime + cook_turn v1.1.0
--
-- Two changes vs v1.0.0:
--
-- 1. `# All recipe steps` block added. The v1.0.0 prompt only showed
--    the CURRENT step; model hallucinated when asked about adjacent
--    steps (observed 2026-04-22: user on step 2 asked about step 3,
--    model invented "sautéing with garlic" when real step 3 was
--    "add kale to boiling water"). Full numbered step list grounds
--    answers for cross-step questions without needing RAG.
--
-- 2. Stronger brevity constraint. The v1.0.0 "1–2 short sentences"
--    rule wasn't binding strictly enough; turn outputs frequently
--    exceeded the 150-token audio cap and were cut off mid-sentence.
--    New wording: "Your ENTIRE spoken response MUST fit in 2 short
--    sentences. Under 6 seconds of speech. Nothing more." — direct,
--    measurable, and aligned with the current max_output_tokens
--    policy (see CLAUDE.md §North-star #8).
--
-- Rollout: v1.1.0 lands with `is_default=TRUE`, v1.0.0 is flipped to
-- `is_default=FALSE` but stays `is_enabled=TRUE` so in-flight
-- sessions keyed on v1.0.0 continue to work through their lifetimes.
-- `rollout_pct=100` on v1.1.0 because the fix is correctness-critical,
-- not an AB test.
...
-- ---------------------------------------------------------------------------
-- v1.1.0 — cook_turn (text fallback)
-- ---------------------------------------------------------------------------
