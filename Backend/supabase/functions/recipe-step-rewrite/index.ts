...
Deno.serve(async (req) => {
  ...
  // ---------------------------------------------------------------------
  // 3. Idempotency cache. Keyed on sub_event_id so the substitution-accept
  //    request and a fast double-tap collapse to one Gemini call.
  // ---------------------------------------------------------------------
  try {
  ...
  const renderedPrompt = renderPrompt(
    activePrompt.template_blob,
    {
      ...
      amount_conversion: body.amount_conversion ?? '',
