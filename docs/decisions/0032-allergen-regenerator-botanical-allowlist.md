...
2. **Daniel reviews `slot_hard_rule_violation` and `replacement_also_invalid` rates during the post-beta tuning sweep.** SCA-206 (/review-2 S8) trimmed the false-specific thresholds that previously sat here ("`slot_hard_rule_violation` > 5% and `replacement_also_invalid` within > 30%") because pre-beta we don't have a PostHog dashboard for either metric, and pinning thresholds without dashboards turns into "specific-looking numbers nobody is checking." When the dashboards land (file under `monitoring/runbooks`), update this trigger to point at them. Until then: ad-hoc review, with this entry as the prompt to look at the regenerator funnel during the first post-beta cost / quality sweep. If regenerator-pass failure is materially elevated, escalate to Option B (extra retry attempt), expand the botanical-safe table to additional allergens, or revisit the validator's matcher semantics.
