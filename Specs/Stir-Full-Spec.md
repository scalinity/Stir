...
| `voice_quota_refund`               | request_id, reason ∈ {no_active_prompt, mint_failed, mint_unexpected_error}, is_refresh, upstream_status? | refund-audit observability (SCA-145) — disambiguates the increment-then-refund path on the placeholder-key CI path from never-incremented (both produce identical `usage_counters` post-state) | AI Ops                  |
...
| `leftovers_dish_selected`          | rank, leftovers_items_count, prompt_version, source_recipe_plan_id, new_recipe_plan_id          | leftovers handoff conversion (SCA-55)      | Core Loop               |
