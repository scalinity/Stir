...
| `notification_history_decode_failed` | state_key, error_description                                                              | scheduler reliability (SCA-374) | Reliability         |
...
- **`notification_history_decode_failed`** — pinned by SCA-374 (CR1-S5 from the SCA-355 review cluster). Fires from `NotificationHistoryStore.load()` when the persisted `[Entry]` blob fails JSONDecoder. A decode failure silently resets the rolling-cap state (returns `[]`), reopening the door to over-cap notifications until the next successful save. Pre-SCA-374 the failure only landed in OSLog (`Logger.notifications.warning` at the same call site, kept), which is invisible to PostHog dashboards — an Entry-shape regression that broke every install would only surface from individual sysdiagnose captures. `state_key` ∈ `{"stir.use_soon.history.v1", "stir.leftovers_followup.history.v1"}` discriminates which scheduler's bucket regressed, so a rollout-side `Entry` migration miss can be scoped to one feature without grepping device logs. `error_description` is `NSError.localizedDescription` from the JSONDecoder throw — the persisted blob carries fire dates + bools (no pantry labels, no recipe titles), so ADR 0009 permits the diagnostic precision; if the Entry shape ever evolves to carry user content, this property must drop to a closed-vocab enum like `RollbackErrorReason` (SCA-367 pattern). Dashboard signal: any sustained nonzero rate on either `state_key` after a release indicates a Codable-shape regression in the matching scheduler — bisect against the release that introduced it and ship a forward-compatible migration before the rolling cap silently breaks.
