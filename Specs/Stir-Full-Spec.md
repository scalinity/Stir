...
- **`pantry_auto_consume_resolved`** — fired once per `cook_session_completed`, after `PantryItemRepository.consumeForRecipe` runs. Per ADR 0029, the consume rule is memory-state-aware: matched-and-`.ephemeral` rows soft-delete, matched-and-`.remembered` rows bump `lastSeenAt`, `.expired`/`.unknown` and unmatched are no-ops. Properties are counts only — never ingredient names — to satisfy ADR 0009's privacy invariant. `substituted_count` = number of effective ingredients that came from an accepted `SubstitutionEvent` rather than the original recipe ingredient. Dashboard signal: if `unmatched / (ephemeral_deleted + remembered_bumped + unmatched) > 0.5` for a two-week window, the matcher in `fetchExisting` needs hardening — escalation trigger noted in ADR 0029 §"Trigger to revisit". The event fires unconditionally (even when all five counts are zero) so dashboard time-series stays continuous and a missing emission flags a wiring regression.
