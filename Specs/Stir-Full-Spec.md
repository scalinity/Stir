...
- **`tutorial_started` / `tutorial_step_advanced` / `tutorial_completed` / `tutorial_skipped`** — in-app feature tours (SCA-5), distinct from the setup-onboarding funnel (`onboarding_started` / `onboarding_completed`). `tutorial_id` is a snake_case identifier matching `TutorialKey.rawValue` (e.g. `tonight_tour`). `from_step` / `to_step` carry per-step telemetry IDs (snake_case enum names — `intro`, `solve`, `saved`, `settings` for the Tonight tour). Lifecycle: exactly one `tutorial_started` per presentation, then exactly one of `{tutorial_completed, tutorial_skipped}` on resolution; `tutorial_step_advanced` fires on every intra-tour advance (NOT on the final-step → completed transition — that's covered by `tutorial_completed` alone). Backgrounding mid-tour without resolution is a known third state — funnel queries should compute abandonment as `count(tutorial_started) − count(tutorial_completed) − count(tutorial_skipped)`.
