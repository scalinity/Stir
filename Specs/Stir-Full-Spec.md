...
| `notification_schedule_rollback_failed` | scheduler_id, identifier, error_reason, prior_existed                                   | scheduler reliability (SCA-309/367/369) | Reliability         |
| `notification_history_decode_failed` | state_key, error_reason                                                                   | scheduler reliability (SCA-374/398) | Reliability         |
...
| `step_text_slug_cleanup_completed` | steps_scanned, steps_updated                                                                    | one-shot legacy slug-prose cleanup audit (SCA-430)  | Reliability             |
...
- **`step_text_slug_cleanup_completed`** — pinned by SCA-430. Fires from `StepTextSlugCleanupMigration.runIfNeeded()` after a successful one-shot scan over `RecipeStep.instructionText` rows in the local Core Data store. Background: SCA-423 fixed the source of legacy `KitchenEquipment.CommonCode` slugs (e.g. `food_processor`) leaking into recipe prose at the four AI prompt-render sites, but rows generated before that deploy still carry the raw slug — and `RecipeStep` rows live in CloudKit (north-star constraint #3), unreachable from the backend, so the cleanup must run on-device. The migration walks every `RecipeStep`, applies a word-boundary regex (`\b<slug>\b`) per multi-token slug, and rewrites matches to the lowercased displayName (`food_processor` → `food processor`). Single-token slugs (`oven`, `grill`, `blender`, `microwave`, `stovetop`, `griddle`, `skillet`) are excluded by the `slug.contains("_")` gate because they're valid English words and would false-positive on natural prose. The 10 participating multi-token slugs are `food_processor`, `air_fryer`, `instant_pot`, `slow_cooker`, `stand_mixer`, `rice_cooker`, `cast_iron`, `nonstick_pan`, `sheet_pan`, `dutch_oven`. One-shot per install — the UserDefaults flag `com.scalinity.stir.migrations.stepTextSlugCleanup.v1` gates subsequent runs; reinstall (or test-only `reset()`) re-runs the work under the same idempotent semantics. Foreground-triggered from `RootView`'s scenePhase `.active` block (same hook as `PantryTombstoneReaper`). Emitted on every successful run — including zero-row passes — so dashboards see "migration completed on this install" not "migration found something to fix." Failed runs (Core Data fault during fetch/save) emit nothing AND don't move the flag, so the next foreground retries. Properties: `steps_scanned` is the integer count of `RecipeStep` rows visited (no household filter — the migration scans the entire local store); `steps_updated` is the integer count of rows whose `instructionText` actually changed. Counts only — no instruction prose — per ADR 0009's privacy invariant. Dashboard signal: a sudden non-zero `steps_updated` p95 across the install base after deploy validates the migration reached legacy users; a steady `steps_updated == 0` cohort thereafter confirms the source fix (SCA-423) is holding new generations clean.
...
- **`notification_schedule_rollback_failed`** — pinned by SCA-309; `reactivation` enum value added by SCA-318; `prior_existed` property + emit-on-no-prior branch added by SCA-369; `scheduler_id` typed as a Swift enum by SCA-360; `error_description` (raw OS-supplied) replaced by `error_reason` (closed-vocab `RollbackErrorReason` enum) by SCA-367. Fires from `NotificationSchedulerKit.addWithRollback` on TWO terminal-failure branches (SCA-369 split): (a) primary `UNUserNotificationCenter.add(...)` throws AND rollback re-add of prior request ALSO throws (regression — user HAD a schedule and we lost it); (b) primary `add(...)` throws AND no prior to restore (fresh-user first-schedule failure). `LeftoversFollowupScheduler`, `UseSoonScheduler`, and (since SCA-318) `ReactivationScheduler` all wire the same callback via `NotificationSchedulerKit.defaultRollbackFailureHandler(telemetry:)` (SCA-361), discriminated by `scheduler_id` ∈ `{"leftovers_followup", "use_soon", "reactivation"}`. `identifier` is the failing `UNNotificationRequest.identifier` the kit tried to add (e.g. `"stir.leftoversFollowup.<sessionId>"`); `error_reason` ∈ `{"invalidContent", "deniedByDevice", "systemUnavailable", "unknown"}` (SCA-367 closed-vocab) — pre-fix the kit forwarded raw `error.localizedDescription` strings to PostHog, which accumulated OS-state breadcrumbs at indefinite retention. Raw `localizedDescription` now lives in OSLog only (where `.private` already redacts). `prior_existed` (SCA-369) — `true` on regression branch, `false` on cold-start branch — distinguishes the two so PostHog dashboards don't conflate them. Dashboard signal: a sustained rate with `prior_existed=true` AND `error_reason=systemUnavailable` signals a notification-subsystem fault; `error_reason=deniedByDevice` between primary and rollback usually means a notification-auth flip mid-flight; `error_reason=unknown` warrants Sentry sampling to enrich the closed-vocab. Single-success branches do NOT emit.
- **`notification_history_decode_failed`** — pinned by SCA-374 (CR1-S5 from the SCA-355 review cluster); closed-vocab `error_reason` introduced by SCA-398 (CR3-W4 / SA3-W3 from the SCA-355 review-orchestrator pass). Fires from `NotificationHistoryStore.load()` when the persisted `[Entry]` blob fails JSONDecoder. A decode failure silently resets the rolling-cap state (returns `[]`), reopening the door to over-cap notifications until the next successful save. Pre-SCA-374 the failure only landed in OSLog (`Logger.notifications.warning` at the same call site, kept), which is invisible to PostHog dashboards — an Entry-shape regression that broke every install would only surface from individual sysdiagnose captures. `state_key` ∈ `{"stir.use_soon.history.v1", "stir.leftovers_followup.history.v1"}` discriminates which scheduler's bucket regressed. `error_reason` ∈ `{"dataCorrupted", "keyNotFound", "typeMismatch", "valueNotFound", "unknown"}` is `HistoryDecodeErrorReason` rawValue — closed-vocab. SCA-398 retired the prior `error_description` property (raw `NSError.localizedDescription`) for the same reason SCA-367 retired it on `notification_schedule_rollback_failed`: OS-supplied strings accumulate device-state breadcrumbs in PostHog at indefinite retention. Raw description still lives in OSLog (`.private`-redacted) for triage. Dashboard signal: any sustained nonzero rate on either `state_key` after a release indicates a Codable-shape regression in the matching scheduler — `error_reason=keyNotFound` is the canonical signal of a missing field added in the post-release Entry; `error_reason=typeMismatch` indicates a field type change. Bisect against the release that introduced it and ship a forward-compatible migration before the rolling cap silently breaks.
