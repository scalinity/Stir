...
- **`tutorial_started` / `tutorial_step_advanced` / `tutorial_completed` / `tutorial_skipped`** — in-app feature tours (SCA-5), distinct from the setup-onboarding funnel (`onboarding_started` / `onboarding_completed`). `tutorial_id` is a snake_case identifier matching `TutorialKey.rawValue`. v1 keys: `tonight_tour` (full-screen Tonight Home intro), `scan_capture`, `scan_review`, `dinner_options`, `dish_preview`, `cook_mode_tap`, `voice_mode` (contextual coach-mark sequences attached to feature surfaces). `from_step` / `to_step` carry per-step telemetry IDs (snake_case — e.g. `intro`/`solve`/`saved`/`settings` for the Tonight tour, `framing`/`shutter` for scan capture, `commands` for the voice-command catalog step). Lifecycle: exactly one `tutorial_started` per presentation, then exactly one of `{tutorial_completed, tutorial_skipped}` on resolution; `tutorial_step_advanced` fires on every intra-tour advance (NOT on the final-step → completed transition — that's covered by `tutorial_completed` alone). `tutorial_skipped` carries an optional `reason` property: absent for explicit Skip taps, `"navigated_away"` when the host view disappears mid-tour. Backgrounding mid-tour without resolution is a known third state — funnel queries should compute abandonment as `count(tutorial_started) − count(tutorial_completed) − count(tutorial_skipped)`.
