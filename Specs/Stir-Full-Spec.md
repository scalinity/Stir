...
| `notification_schedule_rollback_failed` | scheduler_id, identifier, error_description, prior_existed                              | scheduler reliability (SCA-309/369) | Reliability             |
...
- **`notification_schedule_rollback_failed`** — pinned by SCA-309; `reactivation` enum value added by SCA-318; `prior_existed` property + emit-on-no-prior branch added by SCA-369; `scheduler_id` typed as a Swift enum by SCA-360. Fires from `NotificationSchedulerKit.addWithRollback` on TWO terminal-failure branches (SCA-369 split): (a) primary `UNUserNotificationCenter.add(...)` throws AND rollback re-add of prior request ALSO throws (regression — user HAD a schedule and we lost it); (b) primary `add(...)` throws AND no prior to restore (fresh-user first-schedule failure). `LeftoversFollowupScheduler`, `UseSoonScheduler`, and (since SCA-318) `ReactivationScheduler` all wire the same callback via `NotificationSchedulerKit.defaultRollbackFailureHandler(telemetry:)` (SCA-361), discriminated by `scheduler_id` ∈ `{"leftovers_followup", "use_soon", "reactivation"}` (matching their existing telemetry-event prefix; adding a new scheduler that delegates to the kit is a wire-contract change — update `NotificationSchedulerKit.swift` callers, this clarification, and the CLAUDE.md telemetry list in the same commit). `identifier` is the failing `UNNotificationRequest.identifier` the kit tried to add (e.g. `"stir.leftoversFollowup.<sessionId>"`); `error_description` is the OS-supplied `NSError.localizedDescription` from the rollback re-add (or, on the no-prior branch, from the primary add) throw — counts only, no user content, satisfies ADR 0009. `prior_existed` (SCA-369) — `true` on regression branch, `false` on cold-start branch — distinguishes the two so PostHog dashboards don't conflate them. Dashboard signal: any non-zero 7-day rolling rate is worth investigating; a sustained rate with `prior_existed=true` signals a system-level notification subsystem fault (auth flipped to `denied` between primary and rollback, NSError thrown from a memory pressure event, etc.) and warrants user-visible recovery copy in the affected scheduler; a `prior_existed=false` rate spike points at upstream initialization (e.g. fresh-user notification authorization race). Single-success branches (`primary add succeeded` OR `rollback re-add succeeded`) do NOT emit.
