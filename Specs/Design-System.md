...
| `icon.hero` | 80pt | Hero icon TILE — the rounded ember-tint square that frames an inner glyph (PantryListView empty, TonightHomeView first-use empty). Distinct from `icon.xl` (the inner glyph); `icon.hero` is the SQUARE, not the symbol inside it. |
